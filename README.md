...
The Blitz3D renderer is based on DirectX 7, which was released in 1999. Although there is nothing technically wrong with DirectX 7 or the Blitz renderer, the reality is that modern graphics drivers have very poor support for this API. Additionally, there is a hard-coded screen size limitation that prevents the API from displaying on resolutions higher than 2048x1152.

To solve this problem, our build uses [WineD3D for Windows](https://fdossena.com/?p=wined3d/index.frag) to translate the game's DirectX 7 calls into OpenGL, which has better driver support. Based on the same technology as [Valve's Proton](https://github.com/ValveSoftware/Proton), this ensures that the game remains free and open-source for all developers. This solution can be used for all versions of the game, not just ours. To enable WineD3D for Windows just copy these two files into your game directory:
- ddraw.dll
- wined3d.dll

There is another problem, though. Laptop computers often use "switchable graphics", with an integrated graphics chip built into the CPU for simple 2D drawing, and a discrete GPU for games. The problem is that by default, any OpenGL application will use the integrated graphics chip, which will cause very slow framerates. In applications written in C/C++ there is a solution for this. We can declare two special variables that will tell the driver that we want to use the discrete GPU. One variable is for Nvidia and the other is for AMD cards:
```c++
extern "C"
{
