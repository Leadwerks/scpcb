...
;-----------------------------------------------------------------------
; Copy default.ini to options.ini if it doesn't already exist. This prevents the saved user settings from getting overridden when an update occurs - Josh

If FileType("options.ini") = 0
	CopyFile("defaults.ini", "options.ini")
EndIf
...
; Interface can display up to a maximum of 24 modes. Easiest solution is to remove the lowest resolution graphics modes, as they are the least likely to be needed. - Josh
Global SkippedGFXModes% = 0
If TotalGFXModes > 24
	SkippedGFXModes = TotalGFXModes - 24
	TotalGFXModes = 24
EndIf

; Worst option, but it always works - Josh
Local DefaultWidth% = GfxModeWidth(SkippedGFXModes + 1)
Local DefaultHeight% = GfxModeHeight(SkippedGFXModes + 1)

; Search for preferred resolution - Josh
For i% = SkippedGFXModes + 1 To CountGfxModes()
	If GfxModeWidth(i) = 1280 And GfxModeHeight(i) = 720
		DefaultWidth = 1280
		DefaultHeight = 720		
	EndIf
Next

; Modified to handle when no default resolution is defined, and we don't know what resolutions are available - Josh
Global GraphicWidth% = GetINIInt(OptionFile, "options", "width", DefaultWidth)
Global GraphicHeight% = GetINIInt(OptionFile, "options", "height", DefaultHeight)

; Make sure this mode is supported - user could have changed monitors - Josh
If GfxModeExists(GraphicWidth, GraphicHeight, 32) = False
	GraphicWidth = DefaultWidth
	GraphicHeight = DefaultHeight
EndIf
...
Global ShowMovies = GetINIInt(OptionFile, "options", "play startup video", "true")
...
Local Hinstance% = SystemProperty( "AppHINSTANCE" )
Local LR_LOADFROMFILE = 16
Local LR_DEFAULTSIZE = 64
Local IMAGE_ICON = 1
Local icon% = ExtractIcon(Hinstance, "SCPCB.exe", 0)
If icon And Hwnd
	Local WM_SETICON% = 128
	Local ICON_BIG% = 1
	Local ICON_SMALL% = 0
	api_SendMessageI(Hwnd, WM_SETICON, ICON_BIG, icon)
	api_SendMessageI(Hwnd, WM_SETICON, ICON_SMALL, icon)
EndIf

;Enables dark mode titlebar on Windows 10
If ENABLE_SCPLIB
	SCPLib_SetDarkMode(Hwnd)
EndIf

;DPI Scale Settings
api_SetProcessDPIAware()
Local dpi = api_GetDpiForWindow(Hwnd)
Global DPIScale = Float(dpi) / 96.0
...
If LauncherEnabled Then 
	...
	Else
		...
		If Fullscreen Then
			DebugLog "Using Fullscreen Mode"; Let's make things absolutely clear --Josh
			Graphics3DExt(GraphicWidth, GraphicHeight, (16*Bit16Mode), 1)
		Else
			DebugLog "Using Windowed Mode"; Again...
...
Else
	For i% = 1 To TotalGFXModes
		...
		For  n% = 0 To TotalGFXModes - 1
			If GfxModeWidths(n) = GfxModeWidth(SkippedGFXModes + i) And GfxModeHeights(n) = GfxModeHeight(SkippedGFXModes + i) Then samefound = True : Exit
		Next
		If samefound = False Then
			If GraphicWidth = GfxModeWidth(SkippedGFXModes + i) And GraphicHeight = GfxModeHeight(SkippedGFXModes + i) Then SelectedGFXMode = GFXModes
			GfxModeWidths(GFXModes) = GfxModeWidth(SkippedGFXModes + i)
			GfxModeHeights(GFXModes) = GfxModeHeight(SkippedGFXModes + i)
			GFXModes=GFXModes+1
		End If
	Next
	...
	Else
		...
		RealGraphicHeight = GraphicHeight
		If Fullscreen Then
			DebugLog "Using Fullscreen"
			Graphics3DExt(GraphicWidth, GraphicHeight, (16*Bit16Mode), 1)
		Else
			DebugLog "Using Window"
			Graphics3DExt(GraphicWidth, GraphicHeight, 0, 2)
		End If
	EndIf
...
If ENABLE_SCPLIB
	SCPLib_SetDarkMode(Hwnd); Enables dark mode titlebar on Windows 10
EndIf

;-------------------------------------------------------------------------------------------------------------------------------
; Initialize Steam after the launcher runs. The notification in the bottom right corner looks bad in the launcher -Josh
;-------------------------------------------------------------------------------------------------------------------------------
If ENABLE_STEAM
	If Steamworks_Initialize() = 1
		DebugLog "Steamworks initialized"	
	Else
		DebugLog "Steamworks failed to initialize"	
	EndIf
EndIf
;-------------------------------------------------------------------------------------------------------------------------------

Global MenuScale# = (GraphicHeight / 1024.0)
Global HUDScale# = Max(Float(GraphicHeight) / 1080.0, 1.0); 100% scaling up to 1080p, and then linear above that, based on vertical resolution - Josh
...
Global LoadMeterIMG% = CopyImage(BlinkMeterIMG); UI scaling for these two images can be slightly different - Josh
...
Repeat
	...
	Else
		...
		If MsgTimer > 0 Then
			...
			If (Not temp%)
				Color 0,0,0
				AAText((GraphicWidth / 2)+1, (GraphicHeight / 2) + 201 * MenuScale, Msg, True, False, Min(MsgTimer / 2, 255)/255.0)
				Color 255,255,255;Min(MsgTimer / 2, 255), Min(MsgTimer / 2, 255), Min(MsgTimer / 2, 255)
				If Left(Msg,14)="Blitz3D Error!" Then
					Color 255,0,0
				EndIf
				AAText((GraphicWidth / 2), (GraphicHeight / 2) + 200 * MenuScale, Msg, True, False, Min(MsgTimer / 2, 255)/255.0)
			Else
				Color 0,0,0
...
Function DrawGUI()
	...
	If HUDenabled Then 
		...
		Color 255, 255, 255	
		Rect (x * HUDScale, y, width * HUDScale, height * HUDScale, False)
		For i = 1 To Int(((width - 2) * (BlinkTimer / (BLINKFREQ))) / 10)
			DrawImage(BlinkMeterIMG, (x + 3 + 10 * (i - 1)) * HUDScale, y + 3.0 * HUDScale)
		Next	
		Color 0, 0, 0
		...
		If EyeIrritation > 0 Then
			Color 200, 0, 0
			Rect((x - 50 - 3) * HUDScale, y - 3 * HUDScale, (30 + 6) * HUDScale, (30 + 6) * HUDScale)
		End If
		
		Color 255, 255, 255
		Rect((x - 50 - 1) * HUDScale, y - 1 * HUDScale, (30 + 2) * HUDScale, (30 + 2) * HUDScale, False)
		...
		For i = 1 To Int(((width - 2) * (Stamina / 100.0)) / 10)
			DrawImage(StaminaMeterIMG, (x + 3 + 10 * (i - 1)) * HUDScale, y + 3.0 * HUDScale)
		Next	
		...
		For  n% = 0 To MaxItemAmount - 1
			...
			If Inventory(n) <> Null Then
				If (SelectedItem <> Inventory(n) Or isMouseOn) Then 
					DrawImage(Inventory(n)\invimg, x + width / 2 - 32 * MenuScale, y + height / 2 - 32 * MenuScale)
				EndIf
			EndIf
			...
			If Inventory(n) <> Null And SelectedItem <> Inventory(n) Then
				...
				If isMouseOn Then
					If SelectedItem = Null Then
						...
						AASetFont Font1
						Color 0,0,0
						AAText(x + width / 2 + 1, y + height + spacing - 15 * MenuScale + 1, Inventory(n)\name, True)							
						Color 255, 255, 255	
						AAText(x + width / 2, y + height + spacing - 15 * MenuScale , Inventory(n)\name, True)	
...
Global HudImagesRescaled% = False
...
Function LoadEntities()
	...
	If Not SprintIcon Then SprintIcon% = LoadImage_Strict("GFX\sprinticon.png")
	If Not BlinkIcon Then BlinkIcon% = LoadImage_Strict("GFX\blinkicon.png")
	If Not CrouchIcon Then CrouchIcon% = LoadImage_Strict("GFX\sneakicon.png")
	If Not HandIcon Then HandIcon% = LoadImage_Strict("GFX\handsymbol.png")
	If Not HandIcon2 Then HandIcon2% = LoadImage_Strict("GFX\handsymbol2.png")
	If Not StaminaMeterIMG Then StaminaMeterIMG% = LoadImage_Strict("GFX\staminameter.jpg")
	
	;Rescale HUD images
	If Not HudImagesRescaled
		HudImagesRescaled = True
		TFormImage BlinkMeterIMG, HUDScale, 0, 0, HUDScale
		TFormImage StaminaMeterIMG, HUDScale, 0, 0, HUDScale
		TFormImage BlinkIcon, HUDScale, 0, 0, HUDScale
		TFormImage CrouchIcon, HUDScale, 0, 0, HUDScale
		TFormImage SprintIcon, HUDScale, 0, 0, HUDScale
	EndIf
...
Function PlayStartupVideos()
	...
	If ShowMovies = 0 Then Return
	;If GetINIInt("options.ini","options","play startup video")=0 Then Return
	
	If ENABLE_CLOSEHANDLER InstallCloseHandler(CLOSEHANDLERKEY, "SCP - Containment Breach v" + VersionNumber)
	...
	Repeat
		...
		If ENABLE_CLOSEHANDLER
			If KeyHit(CLOSEHANDLERKEY)
				quit = True
				Exit
			EndIf
		EndIf
...
Function ProjectImage(img, w#, h#, Quad%, Texture%)
	...
	; This is like image "cover" mode in HTML
	Local scale# = 1
	Local sw# = GraphicsWidth()
	Local sh# = GraphicsHeight()
	If sw >= (sh / 9.0) * 16.0
		scale = (sw / 16.0 * 9.0) / sh
	Else 
		scale = (sh / 9.0 * 16.0) / sw
	EndIf
	
	ScaleEntity Quad, scale * 1.6, scale * 1.6, 1; I can only guess at what B3D might be using as its internal value when zoom equals 1.0 - Josh
	RenderWorld()
