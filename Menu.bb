...
; Functions for GPI scaling
Function ScaledRect(x%, y%, w%, h%, solid% = True)
	;If solid = False And MenuScale > 1.49
	;	Rect(x * MenuScale, y * MenuScale - 1, w * MenuScale, 4)
	;	Rect(x * MenuScale, (y + h) * MenuScale, w * MenuScale, 4)
	;	Rect(x * MenuScale - 1, y * MenuScale, 4, h * MenuScale)
	;	Rect((x + w) * MenuScale - 1, y * MenuScale, 4, h * MenuScale)
	;Else
		Rect(x * MenuScale, y * MenuScale, w * MenuScale, h * MenuScale, solid)
	;EndIf
End Function

Function ScaledText(x%, y%, t$)
	Text x * MenuScale, y * MenuScale, t
End Function

Function ScaledMouseOn(x%, y%, w%, h%)
	Return MouseOn(x * MenuScale, y*MenuScale, w*MenuScale, h*MenuScale)
End Function

Function ScaledDrawTick%(x%, y%, selected% , locked% = False)
	Return DrawTick(x * MenuScale, y * MenuScale, selected, locked)
End Function
...
Function UpdateLauncher()
	...
	Local bold% = MenuScale > 1.49; Larger-resolution text is so thin it's unreadable
	LauncherWidth = LauncherWidth * MenuScale
	LauncherHeight = LauncherHeight * MenuScale
	...
	Font1 = LoadFont_Strict("GFX\font\cour\Courier New.ttf", 18 * MenuScale, bold,0,0)
	SetFont Font1
	MenuWhite = LoadImage_Strict("GFX\menu\menuwhite.jpg")
	MenuBlack = LoadImage_Strict("GFX\menu\menublack.jpg")	
	MaskImage MenuBlack, 255,255,0
	...
	TFormImage LauncherIMG, MenuScale, 0, 0, MenuScale
	...
	; Display available screen resolutions in debug log
	DebugLog "Resolutions:"
	For i = 0 To (GFXModes - 1)
		DebugLog GfxModeWidths(i) + "x" + GfxModeHeights(i)
	Next
	...
	Repeat
		...
		;Cls
		Color 0,0,0
		ScaledRect 0,0,LauncherWidth,LauncherHeight
		...
		ScaledText(20, 240 - 65, "Resolution: ")
		...
		For i = 0 To (GFXModes - 1)
			Color 0, 0, 0
			If SelectedGFXMode = i Then ScaledRect(x - 1, y - 1, 100, 20, False)
			
			ScaledText(x, y, (GfxModeWidths(i) + "x" + GfxModeHeights(i)))
			If ScaledMouseOn(x - 1, y - 1, 100, 20) Then
				Color 100, 100, 100
				ScaledRect(x - 1, y - 1, 100, 20, False)
				If MouseHit1 Then SelectedGFXMode = i
			EndIf
			
			y = y + 20
			;If y >= 250 - 65 + (LauncherHeight / MenuScale - 80 - 260) Then y = 270 - 65 : x=x+100
			
			iy = iy + 1; This will be more reliable across different DPI scales
			If iy >= 6
				iy = 0
				y = 270 - 65
				x=x+100
		...
		x = 30
		y = 369
		ScaledRect(x - 10, y, 340, 95)
		ScaledText(x - 10, y - 25, "Graphics:")
		
		y=y+10
		For i = 1 To CountGfxDrivers()
			Color 0, 0, 0
			If SelectedGFXDriver = i Then ScaledRect(x - 1, y - 1, 290, 20, False)
			;text(x, y, bbGfxDriverName(i))
			LimitText(GfxDriverName(i), x * MenuScale, y * MenuScale, 290 * MenuScale, False)
			If ScaledMouseOn(x - 1, y - 1, 290, 20) Then
				Color 100, 100, 100
				ScaledRect(x - 1, y - 1, 290, 20, False)
		...
		Fullscreen = ScaledDrawTick(40 + 430 - 15, 260 - 55 + 5 - 8, Fullscreen, BorderlessWindowed)
		;BorderlessWindowed = DrawTick(40 + 430 - 15, 260 - 55 + 35, BorderlessWindowed)
		lock% = False

		;If BorderlessWindowed Or (Not Fullscreen) Then lock% = True
		;ShowMovies = DrawTick(40 + 430 - 15, 260 - 55 + 65 + 8, ShowMovies)
		ShowMovies = ScaledDrawTick(40 + 430 - 15, 260 - 55 + 35, ShowMovies)
		;LauncherEnabled = DrawTick(40 + 430 - 15, 260 - 55 + 95 + 8, LauncherEnabled)
		LauncherEnabled = ScaledDrawTick(40 + 430 - 15, 260 - 55 + 65 + 8, LauncherEnabled)
		...
		;EndIf
			...
			ScaledText(40 + 430 + 15, 262 - 55 + 5 - 8, "Fullscreen")
		Color 255, 255, 255
		...
		ScaledText(40 + 430 + 15, 262 - 55 + 35 - 0, "Show movies")
		;Text(40 + 430 + 15, 262 - 55 + 65 + 8, "Show Movies")
		Color 255, 255, 255
		ScaledText(40 + 430 + 15, 262 - 55 + 65 + 8, "Use launcher")
		;Text(40 + 430 + 15, 262 - 55 + 95 + 8, "Use launcher")
		
		If (Not BorderlessWindowed)
			If Fullscreen
				ScaledText(40+ 260 + 15, 262 - 55 + 140, "Current Resolution: "+(GfxModeWidths(SelectedGFXMode) + "x" + GfxModeHeights(SelectedGFXMode) + "," + (16+(16*(Not Bit16Mode)))))
			Else
				ScaledText(40+ 260 + 15, 262 - 55 + 140, "Current Resolution: "+(GfxModeWidths(SelectedGFXMode) + "x" + GfxModeHeights(SelectedGFXMode) + ",32"))
			EndIf
		Else
			...
			If GfxModeWidths(SelectedGFXMode)<G_viewport_width Then
				ScaledText(40+ 260 + 65, 262 - 55 + 160, "(upscaled to")
				ScaledText(40+ 260 + 65, 262 - 55 + 180, G_viewport_width + "x" + G_viewport_height + ",32)")
			ElseIf GfxModeWidths(SelectedGFXMode)>G_viewport_width Then
				ScaledText(40+ 260 + 65, 262 - 55 + 160, "(downscaled to")
				ScaledText(40+ 260 + 65, 262 - 55 + 180, G_viewport_width + "x" + G_viewport_height + ",32)")
			EndIf
		EndIf
		...
		If DrawButton(LauncherWidth - (30 + 90) * MenuScale, LauncherHeight - (50 + 55) * MenuScale, 100 * MenuScale, 30 * MenuScale, "LAUNCH", False, False, False) Then
		...
		If DrawButton(LauncherWidth - (30 + 90) * MenuScale, LauncherHeight - 50 * MenuScale, 100 * MenuScale, 30 * MenuScale, "EXIT", False, False, False) Then End
		Flip
	Forever
	...
	EndIf
	If ShowMovies Then
		PutINIValue(OptionFile, "options", "play startup video", "true")
	Else
	...
	MenuScale# = (GraphicHeight / 1024.0); Restore 
...
Function DrawLoading(percent%, shortloading=False)
	...
	Repeat 
		...
		For  i% = 1 To Int((width - 2) * (percent / 100.0) / 10)
			DrawImage(LoadMeterIMG, x + 3 + 10 * (i - 1), y + 3); Changed from BlitzMeterIMG due to difference in UI scaling - Josh
		Next
		...
		Flip True	; AMD 6600 through DX7 Wine will show a black screen if this is false, only in fullscreen mode - Josh 
					; Reportedly, same result on an Nvidia 3060: https://discord.com/channels/1175951843118031049/1393246372991598613/1458283802672500756
